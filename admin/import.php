...
<head>
<meta http-equiv=\"Content-Type\" content=\"text/html; $codepage\"><title>IMPORT</title>
...
<form class=form-inline action='$htpath/admin/import.php' method='POST' name=submitform>
...
echo "<h4>OK</h4>";
echo "<br><br><b><a href=import.php?speek=$speek>Choose another file / Выбрать другой файл</a></b>";
}
if ($action=="v2") {
echo "<h4>Add all</h4>";
echo ".$base_file";
...
echo "<br><button class=btn type=button onClick=javascript:window.open('$htpath/admin/see.php?file=".rawurlencode($ffff)."','fr','status=no,scrollbars=yes,menubar=no,resizable=yes,location=no,width=800,height=580,left=10,top=10')>File import preview / Просмотреть файл</button><br><br>";
$line1="";
$str=0;
$ff=0;
while(!feof($fp)) {
...
echo "<br><br><form class=form-inline action='$htpath/admin/import.php' method='POST'>
...
<tr><td valign=top>Import from stroke number<br><small>Импортировать с определенной строки</small></td valign=top><td><input size=1 name='strn' type='text' value=\"$strn\"></td><td valign=top> (<i>ex. / например:</i> 0])</td></tr></table>
<input type='hidden' name='language' value='ru'>
<input type='button' value=\"&lt;&lt; ".$lang['back']." / Back\" onclick=\"javascript:history.go(-1)\"> <input type='submit' value='Next / Далее &gt;&gt;'>
</form>";
echo "<br><br><b><a href=import.php?speek=$speek>Choose another file / Выбрать другой файл</a></b>";
...
<form class=form-inline action='$htpath/admin/import.php' method='POST'>
<input type='hidden' name='import' value='true'>
Import from stroke number / Импортировать с определенной строки: <input type='text' name='strn' value='$strn'><br>
...
echo "<h4>DB Import</h4>Data format / Поддерживаемый формат: <b>text/plain</b>, <b>text/richtext</b> separated / с разделителями<br><br><form enctype='multipart/form-data' action='$htpath/admin/import.php' method='POST'>
