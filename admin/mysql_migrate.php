...
$query="DROP TABLE IF EXISTS `".$dbpref."_users`";
//echo "$query ...<br>\n";
mysql_query("$query");
...
$s=0;
$zap="";
$query="INSERT INTO `".$dbpref."_users` SET ";
while (list($key,$val)=each($user_fields)) {
...
echo "<meta http-equiv=\"Refresh\" content=\"2; URL=mysql_migrate.php?speek=$speek&step=1\">";
...
echo "<meta http-equiv=\"Refresh\" content=\"2; URL=mysql_migrate.php?speek=$old_speek&step=2&sp=$sp\">";
...
# ”дал€ем таблицу если она есть
$query="DROP TABLE IF EXISTS `".$dbpref."_items_".$speek."`";
//echo "$query ...<br>\n";
mysql_query("$query");

$query="CREATE TABLE IF NOT EXISTS `".$dbpref."_items_".$speek."` (";
...
$zap="";
$query="INSERT INTO `".$dbpref."_items_".$speek."` SET ";
...
$sp+=1;
$speek=$old_speek;
echo "<meta http-equiv=\"Refresh\" content=\"2; URL=mysql_migrate.php?speek=$speek&step=1&sp=$sp\">";
}
}
...
# ”дал€ем таблицу если она есть
$query="DROP TABLE IF EXISTS `".$dbpref."_orders`";
//echo "$query ...<br>\n";
mysql_query("$query");

$query="CREATE TABLE IF NOT EXISTS `".$dbpref."_orders` (";
...
$s=0;
$zap="";
$query="INSERT INTO `".$dbpref."_orders` SET ";
while (list($key,$val)=each($order_fields)) {
$query.="$zap"."`".mysql_real_escape_string($key)."`". "='". mysql_real_escape_string($tmpm[$s])."'";
echo "\n";
$zap=",";
...
$s=0;
$zap="";
$query="INSERT INTO `".$dbpref."_orders_closed` SET ";
while (list($key,$val)=each($order_fields)) {
$query.="$zap"."`".mysql_real_escape_string($key)."`". "='". mysql_real_escape_string($tmpm[$s])."'";
echo "\n";
$zap=",";
...
echo "<meta http-equiv=\"Refresh\" content=\"2; URL=mysql_migrate.php?speek=$speek&step=3&sp=$sp\">";
}
...
# ”дал€ем таблицу если она есть
$query="DROP TABLE IF EXISTS `".$dbpref."_search_tags`";
//echo "$query ...<br>\n";
mysql_query("$query");

$query="CREATE TABLE IF NOT EXISTS `".$dbpref."_search_tags` (`md5_cache` TEXT";
...
$s=0;
$zap=",";
$query="INSERT INTO `".$dbpref."_search_tags` SET `md5_cache`='".mysql_real_escape_string($indx)."'";
reset ($tags_fields);
while (list($key,$val)=each($tags_fields)) {
$query.="$zap"."`".mysql_real_escape_string($key)."`". "='". mysql_real_escape_string(trim(@$tmpm[$s]))."'";
echo "\n";
...
echo "<meta http-equiv=\"Refresh\" content=\"2; URL=mysql_migrate.php?speek=$speek&step=4&sp=$sp\">";
}
