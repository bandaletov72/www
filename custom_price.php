...
if (($dd==0)||($dd==5)||($dd==6)||($dd==7)||($dd==8)||($dd==9)) { $daystogo.=" дней назад";}
if (($dd==2)||($dd==3)||($dd==4)) { $daystogo.=" дня назад";}
}
if ($days==-0) { $daystogo="Приход груза: Завтра"; }
if ($days==0) { $daystogo="Приход груза: Сегодня"; }
return $str."<br>$daystogo";
...
$dtop=Array();
$dhid=Array();
if (file_exists("./templates/$template/$speek/custom_cart.inc")) {
$cc=file("./templates/$template/$speek/custom_cart.inc");
while (list ($kcc, $vcc) = each ($cc)) {
$vcc=trim($vcc);
if ($vcc!="") {
$ou=explode("|", $vcc);
$dnam[(17+$kcc)]=$ou[1];
...
echo "<div class=alert>Not exists ./templates/$template/$speek/custom_cart.inc !</div>";
}
unset($tmp);
$cpf_t="";
$cpf=Array();
if (file_exists("./templates/$template/custom_price.inc")) {
$cpf=file("./templates/$template/custom_price.inc");
reset($cpf);
$j=0;
while (list ($kcpf, $vcpf) = each ($cpf)) {
$vcpf=trim($vcpf);
if ($vcpf!="") {
$tmpv=explode(":",$vcpf);
if (trim($tmpv[1])!="") {
...
if (count($tmpv2)>1) {
reset ($tmpv2);
while (list ($kcpf2, $vcpf2) = each ($tmpv2)) {
//echo $vcpf2."<br>";
$vcpf2=trim($vcpf2);
$in=doubleval($vcpf2);
if (preg_match("/\*/", $vcpf2)) {
@$outc[$in]=dformat(@$outc[$in]);
}
...
if ($ttm!="") { $cpf_v.="<table class=\"table2 table-condensed table-bordered\"><tbody>$top".$ttm."</tbody></table> ";}
unset($tmz,$ttm);
} else {
if (@$dnam[$in]!="") {
...
$proceed=1;
$in=doubleval(trim($tmpv[1]));
if (preg_match("/\*/", trim($tmpv[1]))) {
@$outc[$in]=dformat(@$outc[$in]);
}
...
if ($tax_function==1) { $colsp=6+$j; } else {$colsp=5+$j;}
while (list ($keyds, $stds) = each ($tmp_dirsub)) {
echo "</tbody></table><table border=1 width=100% cellspacing=0 cellpadding=5 class=\"table2 table-bordered table-striped\"><tr><td colspan=$colsp class=\"panel\"><b>".str_replace("|", " / ", $stds)."</b></td></tr>\n
<tr><td bgcolor=\"#f2f2f2\" align=\"center\" width=20px><b>#</b></td><td bgcolor=\"#f2f2f2\"><b>".$lang['name']."</b></td>$cpf_t<td bgcolor=\"#f2f2f2\" align=\"center\"><b>%</b></td>";
if ($tax_function==1) {echo "<td bgcolor=\"#f2f2f2\" align=\"center\"><b>".$lang[780].",%</b></td><td bgcolor=\"#f2f2f2\" align=\"center\"><b>".$lang[781].",".$currencies_sign[$_SESSION["user_currency"]]."</b></td>"; }
echo "<td bgcolor=\"#f2f2f2\" align=\"center\"><b>".$lang['price'].",".$currencies_sign[$_SESSION["user_currency"]]."</b></td></tr>\n\n";
$tmp_tmp=explode ("\n", $tmp_massive["$stds"]);
natcasesort($tmp_tmp);
reset($tmp_tmp);
...
$jj--;
$ind=(10+$jj);
$cpf_s="<td align=\"center\">".@$stnew[$ind]."</td>".$cpf_s;
