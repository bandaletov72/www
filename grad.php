        ...
        switch($direction) {
            ...
            case 'vertical':
               $line_numbers = round(imagesy($im)*1);
                ...
                list($r1,$g1,$b1) = $this->hex2rgb($start);
                list($r2,$g2,$b2) = $this->hex2rgb($end);
                $r3=$r1+70; if ($r3>255) {$r3=255; }
                $g3=$g1+70; if ($g3>255) {$g3=255; }
                $b3=$b1+70; if ($b3>255) {$b3=255; }
                break;
            case 'ellipse':
                $width = imagesx($im);
                $height = imagesy($im);
...
if (!isset($_GET['s'])){$_GET['s']="ccc";}  if (!preg_match("/^[a-zA-Z0-9]+$/i",$_GET['s'])) { $_GET['s']="ccc";}
if (!isset($_GET['e'])){$_GET['e']="fff";}  if (!preg_match("/^[a-zA-Z0-9]+$/i",$_GET['e'])) { $_GET['e']="fff";}
