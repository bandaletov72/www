...
if(isset($_GET['map'])) {$map=$_GET['map']; }elseif(isset($_POST['map'])){ $map=$_POST['map']; }else {$map="3";}
if (!preg_match('/^[0-9]+$/i',$map)) { $map="2";}
...
$shd=0;
if (($curstatus==trim($statuses[2]))||($curstatus==trim($statuses[0]))||($curstatus==trim($statuses[1]))||($curstatus=="")) {
if ($ncc!=0) {
$tt=explode("<br>", $o[$ncc]);
...
$ttime=time();
//echo $o[3]."<br>";
$line_color = ImageColorAllocate ($im, rand(50,200), rand(50,200), rand(50,200));
$strt=0;
$rand=rand(0,1);
$rand2=rand(-10,10);
while(list($kc,$kv)=each($shirota)) {

if ($strt==0) {$sty=$rand+$shirota[$kc]; $stx=$rand+$dolgota[$kc];
//imagecopy($im, $im3, $x/2+$stx*$x/360-2-21, $y/2-$sty*$y/180-2, 0, 0, $w, $h);
$y11=$rand+$shirota[$kc];
$x11=$rand+$dolgota[$kc];
imagestring($im, 2, ($x/2+$x11*$x/360-20), ($y/2-$y11*$y/180), $y11.":".$x11,  $line_color);
imagestring($im, 2, ($rand2+$x/2+$x11*$x/360-20), ($rand2+$y/2-$y11*$y/180+20), $o[3],  $line_color);

} else {
$y11=$shirota[$kc];
$x11=$dolgota[$kc];
//echo "$stx , $sty -&gt; $x11 , $y11<br>";
imageline ($im,$rand+$x/2+$stx*$x/360-21, $rand+$y/2-$sty*$y/180, $rand+$x/2+$x11*$x/360-21, $rand+$y/2-$y11*$y/180, $line_color);
imagestring($im, 2, ($x/2+$x11*$x/360-20), ($y/2-$y11*$y/180), $y11.":".$x11,  $line_color);
$stx=$x11; $sty=$y11;
}
$strt++;
...
//coords
$rand=rand(0,0);
$xxx=$rand+$x/2+$o[$i21]*$x/360+7-28;
$yyy=$rand+$y/2-$o[$i20]*$y/180-10;
//echo "$xxx $yyy ".$o[$i21]." ".$o[$i20]."<br>";
$xxxx=$o[$i21];
$yyyy=$o[$i20];
$ttt="$yyyy:$xxxx";
...
//imageTtfText($im, $fontsize, 0, $x/2+$o[$i21]*$x/360+7, $y/2-$o[$i20]*$y/180+1, $white,  "arial.ttf",$o[3]);
//imageTtfText($im, $fontsize, 0, $xxx, $rand+$y/2-$o[$i20]*$y/180, $fontclr,  "./admin/arial.ttf", $o[3]);
imagestring($im, 2, $xxx, $yyy, $o[3],  $line_color);

if ($o[$i21]<165) {
//imageTtfText($im, $fontsize, 0, $x+$xxx, $rand+$y/2-$o[$i20]*$y/180, $fontclr,  "./admin/arial.ttf", $o[3]);
imagestring($im, 2, $x+$xxx, $yyy, $o[3],  $line_color);
}
if ($o[$i21]>165) {
//imageTtfText($im, $fontsize, 0, $xxx-$x, $rand+$y/2-$o[$i20]*$y/180, $fontclr,  "./admin/arial.ttf", $o[3]);
imagestring($im, 2, $xxx-$x, $yyy, $o[3],  $line_color);
 }
}
}
