...
$titjs="";
if (($catid=="")||($catid=="_")||($catid=="0")) {$file="$base_loc/novelty.txt"; $titjs="<div class=ocat1 align=center><font size=3>".$lang[259]."</font></div><br>";}
$vit_qty=0;
$ff=0;
$str=0;
...
$orderway="";
$ff=0;
$mysql_query="SELECT * FROM $file WHERE (`dir`='".mysql_real_escape_string(@$h[0])."' AND `subdir`='".mysql_real_escape_string(@$h[1])."')";
if (($catid=="")||($catid=="_")||($catid=="0")) {$mysql_query="SELECT * FROM $file";}
$result=mysql_query("$mysql_query");
$total=@mysql_num_rows($result);

$mysql_query="SELECT * FROM $file WHERE (`dir`='".mysql_real_escape_string(@$h[0])."' AND `subdir`='".mysql_real_escape_string(@$h[1])."') ORDER BY `date` DESC LIMIT ".mysql_real_escape_string($sta).",".mysql_real_escape_string($js_max)."";
if (($catid=="")||($catid=="_")||($catid=="0")) {$mysql_query="SELECT * FROM $file ORDER BY `date` DESC LIMIT ".mysql_real_escape_string($sta).",".mysql_real_escape_string($js_max)."";}
//echo $mysql_query;
$result=mysql_query("$mysql_query");
...
$strto=@$strtoma[0];
unset($strtoma);
 $vipold="<span class=oldprice>".($okr*round(@$price/$okr))."</span> "; if ((preg_match("/\%/", @$out[8])==TRUE)&&(doubleval($strto)>0)) { $sales="<table border=0 cellpadding=0 cellspacing=0 width=53 height=53><tr><td bgcolor=$nc10 background=grad.php?h=58&w=10&e=".str_replace("#","",$nc0)."&s=".str_replace("#","",$nc10)."&d=vertical align=center valign=middle><font color=#000000>SALE<BR><b>-".round($strto)."%</b></font></td></tr></table>"; $ueprice=@$ueprice-(@$ueprice*(doubleval($strto))/100); $price=$okr*(round((@$price-(@$price*(doubleval($strto))/100))/$okr));} else { $sales="<table border=0 cellpadding=0 cellspacing=0 width=53 height=53><tr><td bgcolor=$nc10 background=grad.php?h=58&w=10&e=".str_replace("#","",$nc0)."&s=".str_replace("#","",$nc10)."&d=vertical valign=middle align=center>SALE<BR><b>-".$podstavas["$dir|$subdir|"]."%</b></td></tr></table>";
...
$ddt=0;
reset ($sps);
//if (count($sps)<$js_max){ echo "END";}
$start=0;
while ($st < $perpage) {
$gt = 0;
if ((!@$sps[$st]) || (@$sps[$st]=="")): $rem1="<!--"; $rem2="-->"; endif;
if ((!@$sps[$st]) || (@$sps[$st]=="")): $sps[$st]=""; $gt = 1; $rem1="<!--"; $rem2="-->"; break; break; endif;
...
$skl1=strtoken($sps[$st],"|");
$sklname= str_replace($skl1."|", "", $sps[$st]);
$sps[$st]=$skl1;
$stoks="";
$val = $sps[$st]; //см выше
