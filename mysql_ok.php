...
if (!preg_match("/^[a-z0-9_]+$/",$qty)) { $qty=0;}
if(isset($_GET['addqty'])) $addqty=$_GET['addqty']; elseif(isset($_POST['addqty'])) $addqty=$_POST['addqty']; else $addqty=0;
if (!preg_match("/^[a-z0-9_]+$/",$addqty)) { $addqty=0;}
/* Usage :
 *
...
unset($strtoma);
 if ((preg_match("/\%/", @$out_c[8])==TRUE)&&(doubleval($strto)>0)) {
 	...
 	} else {$price=$price-$price*((double)$podstavas["$r|$sub|"]/100);
 	}
    }
...
if ((@$price==0)||(substr(@$out_c[12],0,1)=="0")) {
$catid=""; $error.=$lang['file'].": ".$lang[222]." E6<br>";
} else {
...
//$cart->add_item($unifid."|".$options,$addqty,($price/$kurs),@$out_c[3]." ID:".@$out_c[6], $fid, $options, $optius2, $out_c[9], $okr*(round(($optkurs*$out_c[5])/$okr)), substr(@$out_c[12],1,3) , $out_c[$netweight], trim($out_cart[$fid]),$speek,$out_c[$box_volume]);
...
//echo $unifid."|".$options."-".$addqty."-".($price/$kurs)."-".@$out_c[3]." ID:".@$out_c[6]."-".$fid."-".$options."-".$optius2."-".$out_c[9]."-".$okr*(round(($optkurs*$out_c[5])/$okr))."-".$defvalut."-".$out_c[$netweight]."-".trim($out_cart[$fid])."-".$speek."-".$out_c[$box_volume]."-";
...
$image = new gd_buy($unifid,$addqty,$nc5,$nc0,$rets);
//header("Content-type: image/jpeg");
//imagejpeg($image);
