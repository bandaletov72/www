...
$w=strip_tags($w);
$w=str_replace("  ", " ", str_replace("  ", " ", str_replace("  ", " ", str_replace("  ", " ", str_replace("(", "",str_replace(")", "",str_replace("|", "", $w)))))));
if (!isset($w)){$w="";} $w=trim(stripslashes($w)); if (!preg_match("/^[¨¸à-ÿÀ-ßa-zA-Z0-9_\.\,\?\&\#\;\ \%\/-\[\]\']+$/i",$w)) { $w="";}
//echo "<div class=cat2><b>".$lang['keyword'].":</b> '".$w."'</div><br>";
$queries=Array();
$queries=explode(" ",$w);
$count=count($queries);
$minibase="";
...
$stun=toLower($stun);
$qw="(".implode("|",$queries).")";
if (@preg_match("/".$qw."/i",toLower($stun))) {
$scount+=1; }
...
$newname=str_replace("$lineq", "<b>$lineq</b>", toLower($newname));
}
$ff+=1;
...
//echo $lineq . " ".$ext_id.toLower($stun)."<br>";
$qw="(".implode("|",$queries).")";
if (@preg_match("/".$qw."/i",$ext_id.toLower($stun))) {
...
$out[8]=str_replace("<img", "<img hspace=10 width=20 height=20",@$out[8]);
if ($ff<=100) {
$out[2]=str_replace("["."comm"."]","<!--",str_replace("["."/comm"."]","-->",$out[2]));
if ($hidart==1) {$hidnazv=strtoken(@$out[2],"*")." ".strtoupper($ext_id);
} else {
$hidnazv=@$out[2];
}
$shpr="";
